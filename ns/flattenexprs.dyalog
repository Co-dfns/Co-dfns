FE←{
     ...
     lft←{                                  ⍝ Op; Lift all expressions to scope root
         ...
         lfn←FE⊃⌽¯1↓k                       ⍝ Lifted function in call
         nex←(⊂1↑⍺),(¯2↓k),lfn rex          ⍝ All components of the lifted expr
         nex←(⊃⍪/nex)up ⍵                   ⍝ Lifted call with right arg replacement
         (rlf⍪isn⊃(nam Bind nex)nex)ret     ⍝ Right lifts + Bound call, & Return var
     }
     1=≢⍵:⍵                                 ⍝ Do nothing for leaves
     'Expression'≡⊃0 1⌷⍵:{                  ⍝ Lift root expressions
         'atomic'≡⊃'class'Prop 1↑⍵:⍵        ⍝ Do nothing for atomics
         nam←⊃'name'Prop 1↑⍵                ⍝ Expression binding
         nam←(~∧/' '=nam)⊃'exv'nam         ⍝ Expression binding or filler name
         rlf rex←(⊃⌽k←1 Kids ⍵)(nam lft)⊃⍵  ⍝ Lifted Expression
         ⊃⍪/rlf(1↑⍵),(¯1↓k),⊂rex            ⍝ Recombined node with lifted above
     }⍵
     'Condition'≡⊃0 1⌷⍵:{                   ⍝ Lifting Condition nodes is special
         l←⊃,/(2↑1 Kids ⍵)('cnd'lft)¨0 1+⊃⍵ ⍝ All lifted children and lifted code
