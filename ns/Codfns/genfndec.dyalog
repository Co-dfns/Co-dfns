 GenFnDec←{
     ...
     fr←AddFunction ⍺(fnf,'m')ft          ⍝ Insert Monadic function into module
     _←AddAlias ⍺ ft fr(fnf,'d')          ⍝ Alias dyadic name to the monadic body
     0=≢fnr:fr                            ⍝ Return if no other names
     fr⊣⍺{                                ⍝ Return the function reference
         _←AddAlias ⍺ ft fr(⍵,'m')        ⍝ Alias the monadic version
         AddAlias ⍺ ft fr(⍵,'d')          ⍝ Alias the dyadic version
     }¨fnr                                ⍝ Otherwise, alias the others
 }