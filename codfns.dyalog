:Namespace codfns
⎕IO ⎕ML ⎕WX VERSION AF∆PREFIX AF∆LIB←0 1 3 (2 0 1) '/opt/arrayfire' 'cuda'
...
rtn[22],←⊂' DOB(lc,if(lv[i]<0||lv[i]>=r.s[rr-av[i]-1])err(3))                 //+1106R~',NL
rtn[22],←⊂' z.s=SHP(rr-lc);I j=0;DOB(rr,if(!m[rr-i-1])z.s[j++]=r.s[i])',NL
rtn[22],←⊂' if(rr<5){IDX x[4];DOB(lc,x[rr-av[i]-1]=lv[i]);                    //+1106R~',NL
...
rtn[25],←⊂' VSWITCH(l.v,err(6),if(cl)v.as(s32).host(s.data()),if(cl)err(16))',NL
...
rtn[25],←⊂' VSWITCH(r.v,err(6),z.v=v(iota(cz%cr)),',NL
...
rtn[29],←⊂' VSWITCH(l.v,err(6),v.as(s32).host(lv.data()),err(11))',NL
...
rtn[29],←⊂'  arr rv;VSWITCH(r.v,err(6),rv=unrav(v,r.s),err(16))',NL
...
rtn[29],←⊂'  VSWITCH(r.v,err(6),z.v=v(jx),err(16))}}',NL
...
rtn[32],←⊂'DEFN(dis)',NL
rtn[32],←⊂'MF(dis_f){z.s=eshp;z.v=r.v(0);}',NL
...
rtn[34],←⊂'DF(nqv_f){z.s=eshp;B lr=rnk(l),rr=rnk(r);if(lr!=rr){z.v=scl(1);R;}',NL
...
rtn[37],←⊂' arr lv=tile(arr(sv,rt),lt);IDX x[4];x[k]=0;                       //+1106R~',NL
...
rtn[39],←⊂' if(!cnt(z)){z.v=scl(0);R;}array w=where(x).as(s32);IDX ix[4];     //+1106R~',NL
...
rtn[39],←⊂' if(rc<lv)err(5);rc=(I)(rc-abs(lv));map_o mfn_c(llp);',NL
rtn[39],←⊂' A t(r.s,scl(0));t.s[av]=rc;if(!cnt(t)){z=t;R;}',NL
rtn[39],←⊂' dim4 ts;DO((I)rnk(t),ts[i]=t.s[i])',NL
rtn[39],←⊂' if(!lv){t.v=ll.id(t.s);z=t;z.v=flat(z.v);R;}seq rng(rc);IDX x[4]; //+1106R~',NL
...
rtn[60],←⊂'DEFN(mdv)',NL
rtn[60],←⊂'MF(mdv_f){B rr=rnk(r),rc=cnt(r);arr rv=unrav(r);',NL
