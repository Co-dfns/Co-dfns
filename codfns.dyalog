...
rth,←'#define FP(n) NM(n,"",0,0,MT,MFD,DFD,MT,MT);MF(n##_f){n##fn(z,A(),r);}',NL
rth,←'#define EF(ex,fun,init) EXPORT V ex##_dwa(lp*z,lp*l,lp*r){try{\',NL
rth,←'  A cl,cr,za;if(!is##init){init##fn(za,cl,cr);is##init=1;}\',NL
rth,←'  cpda(cr,r);cpda(cl,l);fun##fn(za,cl,cr);cpad(z,za);}\',NL
rth,←' catch(U n){derr(n);}\',NL
rth,←' catch(exception e){msg=mkstr(e.what());dmx.e=msg.c_str();derr(500);}}\',NL
rth,←'EXPORT V ex##_cdf(A*z,A*l,A*r){try{fun##fn(*z,*l,*r);}catch(U n){derr(n);}\',NL
...
rth,←'dim4 eshp=dim4(0,(B*)NULL);std::wstring msg;',NL
rth,←'',NL
...
rth,←'std::wstring mkstr(const char*s){B c=std::strlen(s);std::wstring t(c,L'' '');',NL
rth,←' mbstowcs(&t[0],s,c);R t;}',NL
...
rtn[18],←⊂'  lorfn(a,a,b);z.v=lv*(rv/((!a.v)+a.v));})',NL
...
rtn[23],←⊂'#define brkop(zz,ll,rr) brk_o zz(ll,rr)',NL
...
rtn[27],←⊂'DF(ctf_f){I x=l.r>r.r?l.r:r.r;if(l.r||r.r){catfn(z,l,r,x-1);R;}',NL
rtn[27],←⊂' A a,b;catfn(a,l);catfn(b,r);catfn(z,a,b,0);}',NL
...
rtn[39],←⊂'',NL
rtn[39],←⊂'#define redop(zz,rr) red_o zz(rr)',NL
rtn[39],←⊂'',NL
...
rtn[39],←⊂' t.v=r.v(rc-1,span);mapop(mfn,ll);',NL
rtn[39],←⊂' DO(rc-1,mfn(t,A(t.r,t.s,r.v(rc-(i+2),span)),t))z=t;}',NL
rtn[39],←⊂'DF(red_o){if(l.r!=0&&(l.r!=1||l.s[0]!=1))err(5);if(!r.r)err(4);',NL
rtn[39],←⊂' I lv=l.v.as(s32).scalar<I>();if((r.s[0]+1)<lv)err(5);',NL
rtn[39],←⊂' I rc=(I)((1+r.s[0])-abs(lv));mapop(mfn,ll);',NL
...
rtn[39],←⊂'  DO(lv-1,mfn(t,A(t.r,t.s,r.v(rng+((D)lv-(i+2)),span)),t))',NL
rtn[39],←⊂' }else{t.v=r.v(rng,span);',NL
rtn[39],←⊂'  DO(abs(lv)-1,mfn(t,A(t.r,t.s,r.v(rng+(D)(i+1),span)),t))}',NL
rtn[39],←⊂' z=t;}',NL
rtn[39],←⊂'',NL
...
rtn[40],←⊂'',NL
rtn[40],←⊂'#define rdfop(zz,rr) rdf_o zz(rr)',NL
rtn[40],←⊂'',NL
rtn[40],←⊂'MF(rdf_o){A t(r.r?r.r-1:0,dim4(1),r.v(0));DO(t.r,t.s[i]=r.s[i])',NL
rtn[40],←⊂' I rc=(I)r.s[t.r];I zc=(I)cnt(t);mapop(mfn,ll);',NL
...
rtn[40],←⊂'  mfn(t,A(t.r,t.s,r.v(x[0],x[1],x[2],x[3])),t));z=t;}',NL
...
rtn[40],←⊂' mapop(mfn,ll);A t(r.r,r.s,scl(0));t.s[ra]=rc;if(!cnt(t)){z=t;R;}',NL
...
rtn[40],←⊂'   mfn(t,A(t.r,t.s,r.v(x[0],x[1],x[2],x[3])),t))',NL
...
rtn[41],←⊂'',NL
rtn[41],←⊂'#define scnop(zz,rr) scn_o zz(rr)',NL
rtn[41],←⊂'',NL
...
rtn[41],←⊂' mapop(mfn,ll);z.v=array(z.s,f64);A t(z.r?z.r-1:0,z.s,r.v(0));',NL
...
rtn[42],←⊂'',NL
rtn[42],←⊂'#define scfop(zz,rr) scf_o zz(rr)',NL
rtn[42],←⊂'',NL
...
rtn[42],←⊂' I tc=(I)cnt(t);af::index x[4];mapop(mfn,ll);',NL
...
rtn[46],←⊂'#define mapop(zz,rr) map_o zz(rr)',NL
rtn[46],←⊂'',NL
...
rtn[47],←⊂'#define comop(zz,rr) com_o zz(rr)',NL
rtn[47],←⊂'',NL
rtn[47],←⊂'MF(com_o){ll(z,r,r);}DF(com_o){ll(z,r,l);}',NL
rtn[47],←⊂'',NL
rtn[48],←⊂'OD(dot,"dot",0,0,MT,DFD)',NL
rtn[48],←⊂'#define dotop(zz,ll,rr) dot_o zz(ll,rr)',NL
rtn[48],←⊂'',NL
...
rtn[49],←⊂' DO((I)sp[cr],sqdfn(tv[i],scl(scl(i)),b);ll(tv[i],tv[i]))',NL
...
rtn[49],←⊂'  sqdfn(ta,ai,a);sqdfn(tb,bi,b);ll(tv[i],ta,tb);',NL
...
rtn[50],←⊂'#define powop(zz,ll,rr) pow_o zz(ll,rr)',NL
rtn[50],←⊂'',NL
...
rtn[51],←⊂'#define jotop(zz,ll,rr) jot_o zz(ll,rr)',NL
rtn[51],←⊂'',NL
...
rtn[56],←⊂'#define oupop(zz,rr) oup_o zz(rr)',NL
rtn[56],←⊂'',NL
...
rtn[56],←⊂' mapop(mfn,ll);A xa(2,ts,x);A ya(2,ts,y);mfn(xa,xa,ya);',NL
