...
BS1∆GCC_TEST←{~(⊂'gcc')∊C.TEST∆COMPILERS:0⊣#.UT.expect←0
  ...
  _←'Scratch/bs1_gcc.c'#.codfns.C.Fix BS
  _←⎕SH './gcc Scratch/bs1_gcc'
...
BS1∆ICC_TEST←{~(⊂'icc')∊C.TEST∆COMPILERS:0⊣#.UT.expect←0
  ...
  _←'Scratch/bs1_icc.c'#.codfns.C.Fix BS
  _←⎕SH './icc Scratch/bs1_icc'
  _←'Run_icc'⎕NA'./Scratch/bs1_icc.so|Run >PP <PP <PP'
  #.UT.expect←interp←L NS.Run R ⋄ C.COMPILER←'gcc'
  Run_icc 0 L R}

BS1∆PGI_TEST←{~(⊂'pgcc')∊C.TEST∆COMPILERS:0⊣#.UT.expect←0
  D←⍉GD 7 ⋄ R←⊃((⎕DR 2↑D)323)⎕DR 2↑D ⋄ L←,¯1↑D ⋄ C.COMPILER←'pgcc'
  _←'Scratch/bs1_pgi.c'#.codfns.C.Fix BS
  _←⎕SH './pgi Scratch/bs1_pgi'
  _←'Run_pgi'⎕NA'./Scratch/bs1_pgi.so|Run >PP <PP <PP'
  #.UT.expect←interp←L NS.Run R ⋄ C.COMPILER←'gcc'
  Run_pgi 0 L R}
