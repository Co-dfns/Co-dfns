...
S,←⊂'CNDP2←{L←|⍵ ⋄ B←⍵≥0'
S,←'(÷(○2)*0.5)×(*(L×L)÷¯2)×{coeff+.×⍵*1+⍳5}¨÷1+0.2316419×L' '}'
...
MK∆TST←{id cmp fn←⍺⍺ ⋄ ls rs←⍵⍵ ⋄ l←⍎ls ⋄ r←⍎rs
  ...
  C.COMPILER←cmp ⋄ CS←('bs3',⍕id)C.Fix S
  nv←l(⍎'NS.',fn)r ⋄ cv←l(⍎'CS.',fn)r
  #.UT.expect←1 ⋄ ∧/,1e¯12>|nv-cv
}

BS3∆GCC_TEST←'' 'gcc' 'Run' MK∆TST 'L' 'R'
BS3∆ICC_TEST←'' 'icc' 'Run' MK∆TST 'L' 'R'
BS3∆VSC_TEST←'' 'vsc' 'Run' MK∆TST 'L' 'R'
BS3∆PGCC_TEST←'' 'pgcc' 'Run' MK∆TST 'L' 'R'
