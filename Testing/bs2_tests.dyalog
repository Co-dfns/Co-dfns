...
MK∆TST←{id cmp fn←⍺⍺ ⋄ ls rs←⍵⍵ ⋄ l←⍎ls ⋄ r←⍎rs
  ...
  C.COMPILER←cmp ⋄ CS←('bs2',⍕id)C.Fix S
  nv←l(⍎'NS.',fn)r ⋄ cv←l(⍎'CS.',fn)r
  #.UT.expect←1 ⋄ ∧/,1e¯12>|nv-cv
}

BS2∆GCC_TEST←''  'gcc'  'Run' MK∆TST 'L' 'R'
BS2∆ICC_TEST←''  'icc'  'Run' MK∆TST 'L' 'R'
BS2∆VSC_TEST←''  'vsc'  'Run' MK∆TST 'L' 'R'
BS2∆PGCC_TEST←'' 'pgcc' 'Run' MK∆TST 'L' 'R'
