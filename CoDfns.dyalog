...
Parse←{
  ...
  ⍝
  ⍝ In short, we only expect to handle the Nl and Nss cases 
  ⍝ here, and any other forms are clearly a SYNTAX ERROR.
...
ParseLineVar←{E SC←⍺
  ...
  ~'Variable' 'Token'∧.≡⍵[0 1;1]:¯1 EmptyAST E
  (,'←')≢⊃'name' Prop 1 4⍴1⌷⍵:¯1 EmptyAST E
...
ParseExpr←{
  ...
  'Variable'≡⊃0 1⌷⍵:⍺ {
    ...
    ~('Token'≡⊃1 1⌷⍵)∧('←'≡⊃'name'Prop 1↑1↓⍵):2 EmptyAST ⍺
...
ParseFunc←{
  ...
  FL←(1↑⍵)⍪(1↑⊖⍵)
  ~(FL[;1]∧.≡⊂'Token')∧((,¨'{' '}')∧.≡'name'Prop FL):2 EmptyAST
  ...
  ⍝ parse
  0≠⊃err ast Ne←⍺ ParseExpr ¯1↓1↓⍵:err EmptyAST
  ast[;0]+←1
