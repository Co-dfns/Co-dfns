...
DA(rot_f){z.f=1;B rr=rnk(r),lr=rnk(l);if(rr>4)err(16);
 ...
 res_c(z,scl(scl(r.s[ra])),l,e);B tc=1;DO(ra,tc*=r.s[i])z.v*=(I)tc;
 cat_c(z,z,e,scl(scl(aa-.5)));z.v=flat(tile(axis(z,ra),1,(U)r.s[ra],1));
 z.s[ra]=r.s[ra];
 dim4 s1(1),s2(1);DO(ra+1,s1[i]=r.s[i])DO((I)rr-ra-1,s2[ra+i+1]=r.s[ra+i+1])
 z.v+=flat(iota(s1,s2));res_c(z,scl(scl(tc*r.s[ra])),z,e);
 z.v=flat(r.v(z.v+(tc*r.s[ra])*flat(iota(s2,s1))));}
