...
DF(trn_f){B lr=rnk(l),rr=rnk(r);if(lr>1||cnt(l)!=rr)err(5);if(rr<=1){z=r;R;}
 VEC<I> lv(rr);if(!isint(l))err(11);
 VSWITCH(l.v,err(6),v.as(s32).host(lv.data()),err(11))
 ...
 if(t&&rr<=4){z.s=SHP(rr);DOB(rr,z.s[rr-lv[i]-1]=r.s[rr-i-1])
  ...
  VEC<I> s(rr);DOB(rr,s[rr-lv[i]-1]=(I)(rr-i-1))
  arr rv;VSWITCH(r.v,err(6),rv=unrav(v,r.s),err(16))
  ...
  VSWITCH(r.v,err(6),z.v=v(jx),err(16))}}
