...
MA(cat_f){B ac=cnt(ax),ar=rnk(ax),rr=rnk(r);if(ac>1&&ar>1)err(4);
 VEC<D> axv(ar);ax.v.as(f64).host(axv.data());
 ...
 if(xt){zr++;z.s=SHP(zr);DOB(rr,z.s[i]=r.s[i])B pt=ac?(B)axv[0]:0;
  DOB(zr-pt,z.s[zr-i-1]=z.s[zr-i-2])z.s[pt]=1;R;}
...
DA(cat_f){B ar=rnk(ax),lr=rnk(l),rr=rnk(r);
 ...
 if(lr&&rr&&std::abs((I)lr-(I)rr)>1)err(4);
 ...
 DO(4,z.s[i]=(lr>=rr||i==fx)*ls[i]+(rr>lr||i==fx)*rs[i]);
...
DF(cat_f){B lr=rnk(l),rr=rnk(r);
 if(lr||rr){cat_c(z,l,r,e,scl(scl((I)(lr>rr?lr:rr)-1)));R;}
