:Namespace T
  (⎕IO ⎕ML ⎕WX)←0 1 3 ⋄ A←##.A ⋄ d←A.d ⋄ t←A.t ⋄ k←A.k ⋄ n←A.n
  rn←⊢,∘↓(1+d)↑⍤¯1(+⍀d∘.=∘⍳1+(⌈/0,d))
  ⍝ rn←{w⊣(3⌷⍉w)⍪←'ref'with↓(1+d)↑⍤¯1+⍀d∘.=⍳1+⌈/0,d←0⌷⍉w←⍵}                        ⍝ Function node references
  rd←{w⊣(3⌷⍉fn⌿w)⍪←'depth'with ¯1++/∧.(=∨0=⊢)∘⍉⍨↑ref(fn←Function ⍵)⌿w←⍵}         ⍝ Function depths
